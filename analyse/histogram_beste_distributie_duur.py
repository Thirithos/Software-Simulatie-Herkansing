...
map_histogrammen = "histogrammen/beste_distributie/duur"
...
def maak_histogram_met_modellen(data_reeks, titel, bestandsnaam):
    if len(data_reeks) < 5: 
        return
    
    plt.figure(figsize=(10, 6))
    x_limiet = np.percentile(data_reeks, 99)
    plt.xlim(0, x_limiet)
    
    plt.hist(data_reeks, bins=100, density=True, edgecolor='black', alpha=0.6, color='steelblue')
    ...
    for component_index in range(gmm_model['aantal_componenten']):
        ...
        # zoals histogram_beste_distributie_starttijd.py
    ...
    plt.title(f"Histogram met Modellen: {titel}")
...
for locatie in locaties:
    for weekdag_index, dag_naam in enumerate(dagen_namen):
        ...
        data_locatie_dag = subset['duur_minuten'].values
        maak_histogram_met_modellen(data_locatie_dag, f"{locatie} - {dag_naam}", f"{map_histogrammen}/{locatie}_{dag_naam}.png")